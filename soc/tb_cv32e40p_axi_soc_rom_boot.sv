...
module tb_cv32e40p_axi_soc_rom_boot;
  ...
  cv32e40p_axi_soc #(
      .ROM_INIT_FILE      ("test_rom_boot.memh"),
    ...
    repeat (1000) begin
      ...
      if (status_seen_q) begin
        if (status_value_q !== 32'h1) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: FAIL status=%h", status_value_q);
          $fatal(1);
        end
        if (ext_instr_req_count != 0) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: external instruction fetch should be idle");
          $fatal(1);
        end
        if (ext_axi_read_count != 0) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: external AXI reads should be zero, got %0d", ext_axi_read_count);
          $fatal(1);
        end
        if (ext_axi_write_count != 1) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: expected single external status write, got %0d", ext_axi_write_count);
          $fatal(1);
        end
        if (qspi_local_read_count != 4) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: expected 4 local QSPI reads, got %0d", qspi_local_read_count);
          $fatal(1);
        end
        if (imem_local_write_count != 4) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: expected 4 local IMEM writes, got %0d", imem_local_write_count);
          $fatal(1);
        end
        if (dut.imem_i.mem_i.mem[0] !== 32'h0000_0000 ||
            dut.imem_i.mem_i.mem[1] !== 32'h0000_0000 ||
            dut.imem_i.mem_i.mem[2] !== 32'h0000_0000 ||
            dut.imem_i.mem_i.mem[3] !== 32'h0000_0000 ||
            dut.imem_i.mem_i.mem[4] !== 32'heeee_eeee) begin
          $display("tb_cv32e40p_axi_soc_rom_boot: IMEM copy result mismatch");
          $fatal(1);
        end
        $display("tb_cv32e40p_axi_soc_rom_boot: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_rom_boot: TIMEOUT");
    $fatal(1);
  end
