...
module tb_cv32e40p_axi_soc_rom_handoff;
  ...
  cv32e40p_axi_soc #(
      .ROM_INIT_FILE          ("test_rom_handoff.memh"),
      .IMEM_INIT_FILE         ("test_imem_dirty.memh"),
      .QSPI_SIM_XIP_ENABLE    (1'b1),
      .QSPI_SIM_XIP_INIT_FILE ("test_flash_imem_app.memh"),
      .QSPI_SIM_XIP_DEPTH_WORDS(16),
  ...
  initial begin
    repeat (2000) begin
      ...
      if (status_seen_q) begin
        if (status_value_q !== 32'h2) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: FAIL status=%h", status_value_q);
          $fatal(1);
        end
        if (ext_instr_req_count != 0) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: external instruction fetch should be idle");
          $fatal(1);
        end
        if (ext_axi_read_count != 0) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: external AXI reads should be zero, got %0d", ext_axi_read_count);
          $fatal(1);
        end
        if (ext_axi_write_count != 1) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: expected single external status write, got %0d", ext_axi_write_count);
          $fatal(1);
        end
        if (qspi_local_read_count != 5) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: expected 5 local QSPI reads, got %0d", qspi_local_read_count);
          $fatal(1);
        end
        if (imem_local_write_count != 5) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: expected 5 local IMEM writes, got %0d", imem_local_write_count);
          $fatal(1);
        end
        if (dut.imem_i.mem_i.mem[0] !== 32'h4000_02b7 ||
            dut.imem_i.mem_i.mem[1] !== 32'h0042_8293 ||
            dut.imem_i.mem_i.mem[2] !== 32'h0020_0313 ||
            dut.imem_i.mem_i.mem[3] !== 32'h0062_a023 ||
            dut.imem_i.mem_i.mem[4] !== 32'h0000_006f) begin
          $display("tb_cv32e40p_axi_soc_rom_handoff: IMEM handoff image mismatch");
          $fatal(1);
        end
        $display("tb_cv32e40p_axi_soc_rom_handoff: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_rom_handoff: TIMEOUT");
    $fatal(1);
  end
