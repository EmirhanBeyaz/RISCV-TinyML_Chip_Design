...
module cv32e40p_top #(
  ...
  localparam logic [31:0] DMEM_ADDR   = SOC_DMEM_BASE_ADDR;
  localparam logic [31:0] EXT_ADDR    = SOC_EXT_TEST_BASE_ADDR;
  localparam logic [31:0] STATUS_ADDR = SOC_EXT_TEST_STATUS_ADDR;
  localparam logic [31:0] LOCAL_DATA  = 32'hCAFE_BABE;
  localparam logic [31:0] EXT_DATA    = 32'h1234_5678;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ST_LOCAL_WRITE_REQ: begin
          data_req_o   <= 1'b1;
          data_we_o    <= 1'b1;
          data_be_o    <= 4'hf;
          data_addr_o  <= DMEM_ADDR;
          data_wdata_o <= LOCAL_DATA;
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_LOCAL_WRITE_RESP;
          end
        end

        ST_LOCAL_WRITE_RESP: begin
          if (data_rvalid_i) begin
            ...
            data_addr_o  <= DMEM_ADDR;
            data_wdata_o <= 32'h0;
            state_q      <= ST_LOCAL_READ_REQ;
          end
        end

        ST_LOCAL_READ_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_LOCAL_READ_RESP;
          end
        end

        ST_LOCAL_READ_RESP: begin
          if (data_rvalid_i) begin
            if (data_rdata_i !== LOCAL_DATA) begin
              $display("tb_cv32e40p_axi_soc_dmem_local: local read mismatch %h", data_rdata_i);
              $fatal(1);
            end
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= EXT_ADDR;
            data_wdata_o <= EXT_DATA;
            state_q      <= ST_EXT_WRITE_REQ;
          end
        end

        ST_EXT_WRITE_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_EXT_WRITE_RESP;
          end
        end

        ST_EXT_WRITE_RESP: begin
          if (data_rvalid_i) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b0;
            data_be_o    <= 4'hf;
            data_addr_o  <= EXT_ADDR;
            data_wdata_o <= 32'h0;
            state_q      <= ST_EXT_READ_REQ;
          end
        end

        ST_EXT_READ_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_EXT_READ_RESP;
          end
        end

        ST_EXT_READ_RESP: begin
          if (data_rvalid_i) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= STATUS_ADDR;
            data_wdata_o <= (data_rdata_i == EXT_DATA) ? STATUS_PASS : STATUS_FAIL;
        ...
        default: state_q <= ST_LOCAL_WRITE_REQ;
      endcase
    end
  end
...
module tb_cv32e40p_axi_soc_dmem_local;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (m_axi_awvalid && m_axi_awready && m_axi_wvalid && m_axi_wready) begin
        ...
        if ((m_axi_awaddr !== EXT_ADDR) && (m_axi_awaddr !== STATUS_ADDR)) begin
          $display("tb_cv32e40p_axi_soc_dmem_local: unexpected external write %h", m_axi_awaddr);
          $fatal(1);
        end
      ...
      if (m_axi_arvalid && m_axi_arready && (m_axi_araddr !== EXT_ADDR)) begin
        $display("tb_cv32e40p_axi_soc_dmem_local: unexpected external read %h", m_axi_araddr);
        $fatal(1);
      end
    end
  end
    ...
    repeat (300) begin
      ...
      if (status_seen_q) begin
        if (status_value_q !== 32'h1) begin
          $display("tb_cv32e40p_axi_soc_dmem_local: FAIL status=%h", status_value_q);
          $fatal(1);
        end
        if (ext_write_count_q !== 2) begin
          $display("tb_cv32e40p_axi_soc_dmem_local: external write count mismatch %0d", ext_write_count_q);
          $fatal(1);
        end
        $display("tb_cv32e40p_axi_soc_dmem_local: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_dmem_local: TIMEOUT");
    $fatal(1);
  end
