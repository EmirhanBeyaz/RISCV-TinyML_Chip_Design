...
module cv32e40p_top #(
  ...
  localparam logic [31:0] UART_CTRL_ADDR = SOC_UART0_BASE_ADDR + 32'h0;
  localparam logic [31:0] UART_TX_ADDR   = SOC_UART0_BASE_ADDR + 32'hc;
  ...
  localparam logic [7:0]  TEST_TX_BYTE   = 8'h5a;
...
module tb_cv32e40p_axi_soc_uart0;
  ...
  always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (m_axi_awaddr !== STATUS_ADDR) begin
        $display("tb_cv32e40p_axi_soc_uart0: unexpected external write %h", m_axi_awaddr);
        $fatal(1);
      end
    ...
    repeat (300) begin
      ...
      if (status_seen_q && tx_seen_q) begin
        if (ext_write_count_q !== 1) begin
          $display("tb_cv32e40p_axi_soc_uart0: external write count mismatch %0d", ext_write_count_q);
          $fatal(1);
        end

        $display("tb_cv32e40p_axi_soc_uart0: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_uart0: TIMEOUT");
    $fatal(1);
  end
  ...
  task automatic wait_for_tx_byte(input logic [7:0] exp_data);
    ...
    begin
      ...
      while (uart0_tx !== 1'b0) @(negedge clk);

      for (bit_idx = 0; bit_idx < 8; bit_idx = bit_idx + 1) begin
        @(negedge clk);
        seen_data[bit_idx] = uart0_tx;
      end

      @(negedge clk);
      if (uart0_tx !== 1'b1) begin
        $display("tb_cv32e40p_axi_soc_uart0: TX stop bit missing");
        $fatal(1);
      end

      if (seen_data !== exp_data) begin
        $display("tb_cv32e40p_axi_soc_uart0: TX byte mismatch %h != %h", seen_data, exp_data);
        $fatal(1);
      end
    end
  endtask
