...
module cv32e40p_top #(
  ...
  localparam logic [31:0] REG_STATUS   = SOC_QSPI_CFG_BASE_ADDR + 32'h008;
  localparam logic [31:0] REG_SCRATCH0 = SOC_QSPI_CFG_BASE_ADDR + 32'h018;
  localparam logic [31:0] STATUS_ADDR  = SOC_EXT_TEST_STATUS_ADDR;
  localparam logic [31:0] EXPECTED_STATUS_MASK = 32'h0000_0002;
  localparam logic [31:0] PASS_CODE    = 32'h1;
  localparam logic [31:0] FAIL_CODE    = 32'h2;
  ...
  typedef enum logic [3:0] {
    ...
    ST_READ_STATUS_RESP,
    ST_READ_SCRATCH_REQ,
    ST_READ_SCRATCH_RESP,
    ST_STATUS_STORE_REQ,
    ST_STATUS_STORE_RESP,
    ST_DONE
  } state_t;
    ...
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ST_WRITE_REQ: begin
          ...
          data_we_o    <= 1'b1;
          data_be_o    <= 4'hF;
          data_addr_o  <= REG_SCRATCH0;
          data_wdata_o <= 32'hA5A5_1234;
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
        ...
        ST_READ_STATUS_REQ: begin
          ...
          data_we_o    <= 1'b0;
          data_be_o    <= 4'h0;
          data_addr_o  <= REG_STATUS;
          data_wdata_o <= 32'h0;
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_READ_STATUS_RESP;
          end
        end

        ST_READ_STATUS_RESP: begin
          if (data_rvalid_i) begin
            status_q      <= data_rdata_i;
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b0;
            data_be_o    <= 4'h0;
            data_addr_o  <= REG_SCRATCH0;
            data_wdata_o <= 32'h0;
            state_q      <= ST_READ_SCRATCH_REQ;
          end
        end

        ST_READ_SCRATCH_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_READ_SCRATCH_RESP;
          end
        end

        ST_READ_SCRATCH_RESP: begin
          if (data_rvalid_i) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hF;
            data_addr_o  <= STATUS_ADDR;
            data_wdata_o <= ((data_rdata_i == 32'hA5A5_1234)
                             && ((status_q & EXPECTED_STATUS_MASK) == EXPECTED_STATUS_MASK))
                            ? PASS_CODE : FAIL_CODE;
            state_q      <= ST_STATUS_STORE_REQ;
          end
        end

        ST_STATUS_STORE_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_STATUS_STORE_RESP;
          end
        end

        ST_STATUS_STORE_RESP: begin
          if (data_rvalid_i) begin
            state_q <= ST_DONE;
          end
        end
...
module tb_cv32e40p_axi_soc_qspi_cfg;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (m_axi_awvalid && m_axi_awready && (m_axi_awaddr == STATUS_ADDR)) begin
        status_seen_q  <= 1'b1;
  ...
  initial begin
    ...
    repeat (1000) begin
      @(posedge clk);
      if (status_seen_q && dut.qspi_init_done) begin
        if (status_value_q !== 32'h1) begin
          $display("tb_cv32e40p_axi_soc_qspi_cfg: fail status %h", status_value_q);
          $fatal(1);
        end
        if (dut.qspi_init_error) begin
          $display("tb_cv32e40p_axi_soc_qspi_cfg: unexpected qspi_init_error");
          $fatal(1);
        end
        if (dut.qspi_cfg_i.scratch0_q !== 32'hA5A5_1234) begin
          $display("tb_cv32e40p_axi_soc_qspi_cfg: scratch0 mismatch %h", dut.qspi_cfg_i.scratch0_q);
          $fatal(1);
        end
        if (ext_write_count_q != 1) begin
          $display("tb_cv32e40p_axi_soc_qspi_cfg: MMIO access leaked to external AXI");
          $fatal(1);
        end
        $display("tb_cv32e40p_axi_soc_qspi_cfg: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_qspi_cfg: TIMEOUT");
    $fatal(1);
  end
