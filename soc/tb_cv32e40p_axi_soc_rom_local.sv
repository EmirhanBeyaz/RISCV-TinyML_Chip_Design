      ...
      unique case (state_q)
        ...
        ST_WAIT0: begin
          if (instr_rvalid_i) begin
            if (instr_rdata_i !== 32'hDEADBEEF) begin
              $display("tb_cv32e40p_axi_soc_rom_local: word0 mismatch %h", instr_rdata_i);
              $fatal(1);
            end
            state_q <= ST_REQ1;
          end
        ...
        ST_WAIT1: begin
          if (instr_rvalid_i) begin
            if (instr_rdata_i !== 32'hCAFEBABE) begin
              $display("tb_cv32e40p_axi_soc_rom_local: word1 mismatch %h", instr_rdata_i);
              $fatal(1);
            end
            $display("tb_cv32e40p_axi_soc_rom_local: PASS");
            $finish;
          end
        end
...
module tb_cv32e40p_axi_soc_rom_local;
  ...
  always @(posedge clk) begin
    if (instr_req) begin
      $display("tb_cv32e40p_axi_soc_rom_local: unexpected external instruction request %h", instr_addr);
      $fatal(1);
    end

    if (m_axi_awvalid || m_axi_arvalid) begin
      $display("tb_cv32e40p_axi_soc_rom_local: unexpected data AXI activity");
      $fatal(1);
    end
  end
  ...
  initial begin
    ...
    $display("tb_cv32e40p_axi_soc_rom_local: timeout");
    $fatal(1);
  end
