...
module cv32e40p_top #(
  ...
  localparam logic [31:0] CFG_REG_LO_ADDR     = SOC_TIMER_BASE_ADDR + 32'h000;
  localparam logic [31:0] TIMER_CMP_LO_ADDR   = SOC_TIMER_BASE_ADDR + 32'h010;
  localparam logic [31:0] TIMER_START_LO_ADDR = SOC_TIMER_BASE_ADDR + 32'h018;
  localparam logic [31:0] STATUS_ADDR         = SOC_EXT_TEST_STATUS_ADDR;
  localparam logic [31:0] STATUS_PASS         = 32'h0000_0001;
  localparam logic [31:0] CFG_IRQ_ONE_SHOT    = 32'h0000_0024;
  localparam logic [31:0] CMP_VALUE           = 32'd5;
    ...
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ST_IDLE: begin
          ...
          data_we_o    <= 1'b1;
          data_be_o    <= 4'hf;
          data_addr_o  <= TIMER_CMP_LO_ADDR;
          data_wdata_o <= CMP_VALUE;
          state_q      <= ST_CMP_REQ;
        end
        ...
        ST_CMP_RESP: begin
          if (data_rvalid_i) begin
            ...
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= CFG_REG_LO_ADDR;
            data_wdata_o <= CFG_IRQ_ONE_SHOT;
            state_q      <= ST_CFG_REQ;
          end
        end
        ...
        ST_CFG_RESP: begin
          if (data_rvalid_i) begin
            ...
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= TIMER_START_LO_ADDR;
            data_wdata_o <= 32'h1;
            state_q      <= ST_START_REQ;
          end
        end
        ...
        ST_WAIT_IRQ: begin
          ...
          if (wait_count_q == 8'd12) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= STATUS_ADDR;
            data_wdata_o <= STATUS_PASS;
...
module tb_cv32e40p_axi_soc_timer;
  ...
  logic        timer_irq_seen_q;
  logic        core_irq_seen_q;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      timer_irq_seen_q  <= 1'b0;
      core_irq_seen_q   <= 1'b0;
    end else begin
      if (timer_irq_lo) begin
        timer_irq_seen_q <= 1'b1;
      end

      if (dut.core_irq[SOC_IRQ_MTI_BIT]) begin
        core_irq_seen_q <= 1'b1;
      end
      ...
      if (m_axi_awvalid && m_axi_awready && m_axi_wvalid && m_axi_wready) begin
        ...
        if (m_axi_awaddr !== STATUS_ADDR) begin
          $display("tb_cv32e40p_axi_soc_timer: unexpected external write %h", m_axi_awaddr);
          $fatal(1);
        end
      ...
      if (m_axi_arvalid && m_axi_arready) begin
        $display("tb_cv32e40p_axi_soc_timer: unexpected external read %h", m_axi_araddr);
        $fatal(1);
      end
    end
  end
  ...
  initial begin
    repeat (400) begin
      @(posedge clk);
      if (status_seen_q && timer_irq_seen_q && core_irq_seen_q) begin
        if (status_value_q !== 32'h1) begin
          $display("tb_cv32e40p_axi_soc_timer: FAIL status=%h", status_value_q);
          $fatal(1);
        end

        if (ext_write_count_q !== 1) begin
          $display("tb_cv32e40p_axi_soc_timer: external write count mismatch %0d", ext_write_count_q);
          $fatal(1);
        end

        repeat (2) @(posedge clk);
        if (timer_busy) begin
          $display("tb_cv32e40p_axi_soc_timer: timer IRQ arrived while busy remained set");
          $fatal(1);
        end

        if (timer_irq_hi !== 1'b0) begin
          $display("tb_cv32e40p_axi_soc_timer: unexpected high timer IRQ");
          $fatal(1);
        end

        if (!dut.core_irq[SOC_IRQ_MTI_BIT] && !core_irq_seen_q) begin
          $display("tb_cv32e40p_axi_soc_timer: timer IRQ never reached core MTI bit");
          $fatal(1);
        end

        $display("tb_cv32e40p_axi_soc_timer: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_timer: TIMEOUT");
    $fatal(1);
  end
