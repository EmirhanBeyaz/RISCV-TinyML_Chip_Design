...
module tb_soc_ai_tinyconv_accel_model;
  ...
  localparam int SMOKE_INPUT_H = 3;
  localparam int SMOKE_INPUT_W = 3;
  localparam int SMOKE_OUT_H = 1;
  localparam int SMOKE_OUT_W = 1;
  localparam int SMOKE_CHANNELS = 2;
  localparam int SMOKE_K_H = 2;
  localparam int SMOKE_K_W = 2;
  localparam int SMOKE_PAD_H = 0;
  localparam int SMOKE_PAD_W = 0;
  localparam int INPUT_BYTES = SMOKE_INPUT_H * SMOKE_INPUT_W;
  ...
  soc_ai_tinyconv_accel #(
      .INPUT_H (SMOKE_INPUT_H),
      .INPUT_W (SMOKE_INPUT_W),
      .OUT_H   (SMOKE_OUT_H),
      .OUT_W   (SMOKE_OUT_W),
  ...
  task automatic check_score(
    ...
    begin
      if (got !== exp) begin
        $display("tb_soc_ai_tinyconv_accel_model: %s mismatch got=%0d exp=%0d",
                 what, got, exp);
        $fatal(1);
      end
    end
    ...
    if (!rst_n) begin
      ...
      if (mem_req && mem_we) begin
        if (mem_be !== 4'hf) begin
          $display("tb_soc_ai_tinyconv_accel_model: bad write strobe %b", mem_be);
          $fatal(1);
        end
        ...
        unique case (mem_addr - OUTPUT_BASE)
          ...
          default: begin
            $display("tb_soc_ai_tinyconv_accel_model: unexpected write addr=%h data=%h",
                     mem_addr, mem_wdata);
            $fatal(1);
          end
        endcase
  ...
  initial begin
    if (!$value$plusargs("input_memh=%s", input_memh_file)) begin
      input_memh_file = "build/ai_model_tool_smoke/smoke_input_vector.memh";
    end
    ...
    if (!done_seen) begin
      $display("tb_soc_ai_tinyconv_accel_model: TIMEOUT state=%0d oh=%0d ow=%0d ch=%0d kh=%0d kw=%0d",
               dut.state_q, dut.oh_q, dut.ow_q, dut.ch_q, dut.kh_q, dut.kw_q);
      $fatal(1);
    end

    if (busy || cycle_count == 32'h0 || write_count != 5) begin
      $display("tb_soc_ai_tinyconv_accel_model: bad finish busy=%b cycles=%0d writes=%0d",
               busy, cycle_count, write_count);
      $fatal(1);
    end

    if (result_class !== AI_GOLDEN_RESULT_CLASS[1:0]) begin
      $display("tb_soc_ai_tinyconv_accel_model: class mismatch got=%0d exp=%0d",
               result_class, AI_GOLDEN_RESULT_CLASS);
      $fatal(1);
    end

    if (output_class_word[1:0] !== AI_GOLDEN_RESULT_CLASS[1:0]) begin
      $display("tb_soc_ai_tinyconv_accel_model: output class word mismatch got=%h exp=%0d",
    ...
    $display("tb_soc_ai_tinyconv_accel_model: PASS class=%0d scores=%0d,%0d,%0d,%0d cycles=%0d",
