...
module cv32e40p_top #(
  ...
  localparam logic [31:0] GPIO_SETGPIO_ADDR = SOC_GPIO_BASE_ADDR + 32'h000;
  localparam logic [31:0] GPIO_OUT0_ADDR    = SOC_GPIO_BASE_ADDR + 32'h020;
  localparam logic [31:0] GPIO_SETDIR_ADDR  = SOC_GPIO_BASE_ADDR + 32'h038;
  ...
  localparam logic [31:0] GPIO_PIN_SEL      = 32'd5;
  localparam logic [31:0] GPIO_SETDIR_VAL   = 32'h0100_0000 | GPIO_PIN_SEL;
  localparam logic [31:0] GPIO_SETGPIO_VAL  = GPIO_PIN_SEL;
  localparam logic [31:0] GPIO_OUT0_EXP     = 32'h0000_0020;
    ...
    if (!rst_ni) begin
      ...
      unique case (state_q)
        ST_IDLE: begin
          ...
          data_we_o    <= 1'b1;
          data_be_o    <= 4'hf;
          data_addr_o  <= GPIO_SETDIR_ADDR;
          data_wdata_o <= GPIO_SETDIR_VAL;
          state_q      <= ST_SETDIR_REQ;
        end

        ST_SETDIR_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_SETDIR_RESP;
          end
        end

        ST_SETDIR_RESP: begin
          if (data_rvalid_i) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b1;
            data_be_o    <= 4'hf;
            data_addr_o  <= GPIO_SETGPIO_ADDR;
            data_wdata_o <= GPIO_SETGPIO_VAL;
            state_q      <= ST_SETGPIO_REQ;
          end
        end

        ST_SETGPIO_REQ: begin
          if (data_gnt_i) begin
            data_req_o <= 1'b0;
            state_q    <= ST_SETGPIO_RESP;
          end
        end

        ST_SETGPIO_RESP: begin
          if (data_rvalid_i) begin
            data_req_o   <= 1'b1;
            data_we_o    <= 1'b0;
            data_be_o    <= 4'hf;
            data_addr_o  <= GPIO_OUT0_ADDR;
            data_wdata_o <= 32'h0;
            state_q      <= ST_READ_REQ;
          end
        end
        ...
        ST_READ_RESP: begin
          if (data_rvalid_i) begin
            status_value_q <= (data_rdata_i == GPIO_OUT0_EXP) ? STATUS_PASS : STATUS_FAIL;
            data_req_o     <= 1'b1;
            data_we_o      <= 1'b1;
            data_be_o      <= 4'hf;
            data_addr_o    <= STATUS_ADDR;
            data_wdata_o   <= (data_rdata_i == GPIO_OUT0_EXP) ? STATUS_PASS : STATUS_FAIL;
...
module tb_cv32e40p_axi_soc_gpio;
    ...
    if (!rst_n) begin
      ...
      if (m_axi_awvalid && m_axi_awready && m_axi_wvalid && m_axi_wready) begin
        ...
        if (m_axi_awaddr !== STATUS_ADDR) begin
          $display("tb_cv32e40p_axi_soc_gpio: unexpected external write %h", m_axi_awaddr);
          $fatal(1);
        end
      ...
      if (m_axi_arvalid && m_axi_arready) begin
        $display("tb_cv32e40p_axi_soc_gpio: unexpected external read %h", m_axi_araddr);
        $fatal(1);
      end
    end
  end
    ...
    repeat (300) begin
      ...
      if (status_seen_q) begin
        if (status_value_q !== 32'h1) begin
          $display("tb_cv32e40p_axi_soc_gpio: FAIL status=%h", status_value_q);
          $fatal(1);
        end

        if (ext_write_count_q !== 1) begin
          $display("tb_cv32e40p_axi_soc_gpio: external write count mismatch %0d", ext_write_count_q);
          $fatal(1);
        end

        if (gpio_out[GPIO_PIN] !== 1'b1 || gpio_dir[GPIO_PIN] !== 1'b1) begin
          $display("tb_cv32e40p_axi_soc_gpio: GPIO%0d mismatch out=%0b dir=%0b",
                   GPIO_PIN, gpio_out[GPIO_PIN], gpio_dir[GPIO_PIN]);
          $fatal(1);
        end

        $display("tb_cv32e40p_axi_soc_gpio: PASS");
        $finish;
      end
    end

    $display("tb_cv32e40p_axi_soc_gpio: TIMEOUT");
    $fatal(1);
  end
